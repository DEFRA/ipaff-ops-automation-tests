...
namespace Defra.UI.Tests.Steps
{
    ...
    public class ConfirmationOfDefaultRateChangeSteps
    {
        ...
        public void ThenTheConfirmationOfDefaultRateChangePageShouldBeDisplayedWithTheFollowingSMSExportRateDetails(Table table)
        {
            Assert.True(confirmationOfDefaultRateChangePage?.IsPageLoaded(), "Confirmation of default rate change page is not displayed");

            var actual = confirmationOfDefaultRateChangePage!.GetSmsExportRateConfirmationDetails();
            foreach (var row in table.Rows)
            {
                var field = row["Field"];
                var expected = row["Value"];
        ...
        [Then("the SMS \\(Export) rate should be changing to the value noted at the start")]
        public void ThenTheSMSExportRateShouldBeChangingToTheValueNotedAtTheStart()
        {
            Assert.True(confirmationOfDefaultRateChangePage?.IsPageLoaded(), "Confirmation of default rate change page is not displayed");

            var originalRate = (int)_scenarioContext["SmsExportRate"];
            var expected = $"{originalRate}%";

            var details = confirmationOfDefaultRateChangePage!.GetSmsExportRateConfirmationDetails();
            Assert.IsTrue(details.Values.Any(v => v?.Trim() == expected),
                $"Expected SMS (Export) 'To' value of '{expected}' not found in confirmation details. Actual: {string.Join(", ", details.Select(kv => $"{kv.Key}={kv.Value}"))}");
        }
