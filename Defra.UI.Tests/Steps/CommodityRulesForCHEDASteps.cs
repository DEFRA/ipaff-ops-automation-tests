...
namespace Defra.UI.Tests.Steps.IPAFF
{
    ...
    public class CommodityRulesForCHEDASteps
    {
        ...
        private ICommodityRulesForCHEDAPage? page =>
        ...
        {
            Assert.True(page?.IsPageLoaded(), "Commodity rules for CHED-A page is not displayed");
        }
        ...
        {
            Assert.True(page?.IsPageLoaded(), "Commodity rules for CHED-A page is not displayed");
            Assert.True(page?.WaitForFirstInProgressStatus(expectedStatus),
        ...
        {
            page?.ClickConfirmAndSubmitLinkForFirstInProgress();
        }
        ...
        {
            Assert.True(page?.WaitForFileSubmissionInProgressSectionToBeRemoved(),
                "File submission in progress section was not removed within the timeout");
            Assert.AreEqual(expectedStatus, page?.GetFirstPreviousSubmissionStatus(),
                $"Expected first Previous submission status '{expectedStatus}' but was '{page?.GetFirstPreviousSubmissionStatus()}'");
        }
        ...
        {
            page?.ClickViewSummaryLinkForFirstPreviousSubmission();
        }
