...
{
    public class CheckAndSubmitCommodityRulesForCHEDAPage : ICheckAndSubmitCommodityRulesForCHEDAPage
    {
        private readonly IObjectContainer _objectContainer;
        private IWebDriver _driver => _objectContainer.Resolve<IWebDriver>();

        #region Page Objects
        private IWebElement pageHeading => _driver.WaitForElement(By.XPath("//h1[normalize-space()='Check and submit commodity rules for CHED-A']"), true);
        ...
        public CheckAndSubmitCommodityRulesForCHEDAPage(IObjectContainer container) => _objectContainer = container;

        public bool IsPageLoaded() => pageHeading.Text.Trim().Equals("Check and submit commodity rules for CHED-A");
        ...
        public int GetSummaryFieldAsInt(string field)
        {
            ...
            if (!details.TryGetValue(field, out var raw))
                throw new NoSuchElementException($"Summary field '{field}' not found on Check and submit commodity rules for CHED-A page.");
