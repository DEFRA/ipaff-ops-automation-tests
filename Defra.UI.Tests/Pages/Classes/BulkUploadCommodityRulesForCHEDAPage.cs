...
{
    public class BulkUploadCommodityRulesForCHEDAPage : IBulkUploadCommodityRulesForCHEDAPage
    {
        private readonly IObjectContainer _objectContainer;
        private IWebDriver _driver => _objectContainer.Resolve<IWebDriver>();

        #region Page Objects
        private IWebElement pageHeading => _driver.WaitForElement(By.XPath("//h1[normalize-space()='Bulk upload commodity rules for CHED-A']"), true);
        ...
        public BulkUploadCommodityRulesForCHEDAPage(IObjectContainer container) => _objectContainer = container;

        public bool IsPageLoaded() => pageHeading.Text.Trim().Equals("Bulk upload commodity rules for CHED-A");

        public void ClickChooseFileButton()
        {
            // No explicit "Choose file" button in CHED-A; the input itself triggers the dialog when clicked.
            _ = fileInput;
        }
