...
Scenario: User creates a CHEDA notification and copies it as a new notification - SPS-7389
	Given that I navigate to the IPAFF application
	Then I should see type of Gateway login page
	And I have selected 'Sign in with Government Gateway' as login type
	When I click Continue button from How do you want to sign in page
	...
	And the user selects 'No' for Are any road trailers or shipping containers being used to transport the consignment
	And the user selects means of transport to BCP or Port of entry 'Road vehicle'
	And the user enters transport identification 'TRANS111'
	And the user enters transport document reference 'DOC111'
	And the user enters arrival date at BCP or Port of entry as today's date
	And the user enters estimated arrival time at BCP with future time
	...
	Then the Transport after the BCP or Port of entry page should be displayed
	When the user selects means of transport after BCP 'Road vehicle'
	And the user enters transport identification after BCP 'TRANS222'
	And the user enters transport document reference after BCP 'DOC222'
	And the user enters departure date from BCP '2' days later than arrival date
	And the user enters departure time from BCP '14:00'
	And the user clicks Save and continue
	Then the Transporter page should be displayed
	...
	And the user closes the PDF browser tab
	Then the browser tab is closed
	When the user clicks Copy as new for the notification
	Then the Notification Hub page should be displayed
	And the user records the new CHED reference number
	When the user clicks on 'Origin of the import' link
	Then the Origin of the import page should be displayed
	And the Country of origin is copied over as 'Thailand'
	When the user clicks on Save and return to hub
	Then the Notification Hub page should be displayed
	When the user clicks the Commodity hyperlink
	Then the Commodity page should be displayed with the commodity and description entered
	And the commodity code '0103' and description 'Live swine' are copied over
	When the user populates Number of animals as '5'
	And the user populates Number of packages as '5'
	And the user clicks on Save and return to hub on the Commodity page
	Then the Notification Hub page should be displayed
	When the user clicks on 'Transport to the BCP or Port of entry' link
	Then the Transport to the BCP or Port of entry page should be displayed
	And the BCP or Port of entry 'London Borough of Hillingdon Heathrow Airport Imported Food Office - ADADA' is copied over
	And the means of transport to BCP or Port of entry 'Road vehicle' is copied over
	When the user enters transport identification 'TRANS333'
	And the user enters transport document reference 'DOC333'
	And the user enters arrival date at BCP or Port of entry as today's date
	And the user enters estimated arrival time at BCP with future time
	And the user clicks on Save and return to hub
	Then the Notification Hub page should be displayed
	...
	Then the Accompanying documents page should be displayed
	And no document details are copied over
	When the user selects Document type 'Commercial invoice'
	And the user enters Document reference 'INV99999'
	And the user enters date of issue '01''12''2025'
	And the user clicks on Save and return to hub
	Then the Notification Hub page should be displayed
	When the user clicks on 'Animal identification details' link
	Then the Enter animal identification details page should be displayed
	And no animal identification details are copied over
	When the user populates the Ear tag as 'ET9999'
	And the user clicks on Save and return to hub
	Then the Notification Hub page should be displayed
	When the user clicks on 'Consignor or Exporter, Consignee, Importer and Place of Destination' link
	Then the Addresses page should be displayed
	And the traders are copied across
	When the user clicks on Save and return to hub
	Then the Notification Hub page should be displayed
	When the user clicks on 'Transporter' link
	Then the Transporter page should be displayed
	And the transporter is copied across
	When the user clicks Save and return to hub in Transporter page
	Then the Notification Hub page should be displayed
	When the user clicks on 'Transport after the BCP or Port of entry' link
	Then the Transport after the BCP or Port of entry page should be displayed
	And the means of transport after BCP 'Road vehicle' is copied over
	And the estimated total journey time is copied over
	When the user enters transport identification after BCP 'TRANS444'
	And the user enters transport document reference after BCP 'DOC444'
	And the user enters departure date from BCP '2' days later than arrival date
	And the user enters departure time from BCP '16:00'
	And the user clicks on Save and return to hub
	Then the Notification Hub page should be displayed
	When the user clicks on 'Countries the consignment will travel through' link
	Then the Which countries will the consignment travel through? page should be displayed
	And the countries transited 'Kenya' are copied over
	When the user clicks on Save and return to hub
	Then the Notification Hub page should be displayed
	When the user clicks on 'Contact address for consignment' link
	Then the Contact address for consignment page should be displayed without the secondary title
	And the first branch address is displayed as default
	When the user selects a contact address for the consignment
	And the user clicks on Save and return to hub
	Then the Notification Hub page should be displayed
	When the user clicks on 'Review and submit' link
	Then the Review your notification page should be displayed
	And no errors are displayed on the review page
	And the static values from the original notification match the new notification
	And the dynamic values differ between the original and new notification
	When the user clicks Save and continue
	Then the Declaration page should be displayed
	When the user ticks the checkbox to declare that the information is true and correct
	And the user clicks Submit notification
	Then the Confirmation page should be displayed with the initial risk assessment
	When the user clicks View or print notification
	Then the certificate should be displayed in a new browser tab
	When the user verifies the copied notification data matches the new values entered
	And the user verifies static values match the original notification
	And the user verifies dynamic values differ from the original notification
	And the user closes the PDF browser tab
	Then the browser tab is closed
	When the user logs out of IPAFFS Part 1
	Then the user should be logged out successfully