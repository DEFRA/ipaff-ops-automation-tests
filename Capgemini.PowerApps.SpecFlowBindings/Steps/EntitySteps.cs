...
public class EntitySteps : PowerAppsStepDefiner
{
    ...
    public static void WhenIEnterInTheField(string fieldValue, string fieldName, string fieldType, string fieldLocation, int index = 0)
    {
        if (fieldLocation == "field")
        {
            SetFieldValue(fieldName, fieldValue.ReplaceTemplatedText(), fieldType, index);
        }
        else
        {
    ...
    {
        XrmApp.Entity.GetFormNotifications().Should().NotContain(formNotification => formNotification.Type == FormNotificationType.Information && formNotification.Message == message);
    }
    ...
    public static String ThenIGetTheHeaderTitle()
    {
        return XrmApp.Entity.GetHeaderTitle();
    }
    ...
    private static void SetFieldValue(string fieldName, string fieldValue, string fieldType, int index = 0)
    {
        switch (fieldType)
        {
            case "multioptionset":
